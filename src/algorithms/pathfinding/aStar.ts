...
export const aStar = (
  ...
  const base = grid[startTile.row][startTile.column]; // Get the start tile from the grid
  base.distance = 0; // Set the distance of the start tile to 0
  functionCost[base.row][base.column] =
    base.distance + heuristicCost[base.row][base.column]; // Calculate the initial function cost for the start tile
  ...
  while (untraversedTiles.length > 0) {
    ...
    untraversedTiles.sort((a, b) => {
      ...
      if (functionCost[a.row][a.column] === functionCost[b.row][b.column]) {
      ...
      return functionCost[a.row][a.column] - functionCost[b.row][b.column]; // Sort by function cost
    });
    ...
    if (currentTile) {
      ...
      for (let i = 0; i < neighbors.length; i += 1) {
        ...
        if (distanceToNeighbor < neighbors[i].distance) {
          ...
          functionCost[neighbors[i].row][neighbors[i].column] =
            neighbors[i].distance +
            heuristicCost[neighbors[i].row][neighbors[i].column]; // Update the function cost for the neighbor
  ...
  let current = grid[endTile.row][endTile.column]; // Start from the end tile
  while (current !== null) {
